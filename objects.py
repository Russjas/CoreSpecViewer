...
if __name__ == '__main__':
    ...
    May = RawObject.from_Lumo_directory('C:/Users/russj/Desktop/Stellenbosch/20250530 (1)/20250530/SWIR/Calibration_2_0m00_1m00_250530-134433')
    October = RawObject.from_Lumo_directory('C:/Users/russj/Desktop/Stellenbosch/20251010 (1)/20251010/SWIR/brg10_1_0m00_1m00_251010-104232')
    #%
    from spectral.io import envi
    import spectral as sp
    may_white = envi.open(May.files['white head'])
    may_dark = envi.open(May.files['dark head'])
    oct_white = envi.open(October.files['white head'])
    oct_dark = envi.open(October.files['dark head'])
    #sp.imshow(may_dark)
    #sp.imshow(oct_dark)
    m_w = np.array(may_white.load())
    o_w = np.array(oct_white.load())
    m_d = np.array(may_dark.load())
    o_d = np.array(oct_dark.load())
    
    import matplotlib.pyplot as plt
    m_dmin = np.mean(m_d, axis=0)
    m_dmin = np.mean(m_dmin, axis=0)
    m_wmin = np.mean(m_w, axis=0)
    m_wmin = np.mean(m_wmin, axis=0)
    o_dmin = np.mean(o_d, axis=0)
    o_dmin = np.mean(o_dmin, axis=0)
    o_wmin = np.mean(o_w, axis=0)
    o_wmin = np.mean(o_wmin, axis=0)
    plt.figure()
    plt.plot(m_dmin, label = "may dark")
    plt.plot(o_dmin, label = "oct dark")
    plt.legend()
    plt.title('Darkrefs')
    plt.figure()
    plt.plot(m_wmin, label = "may white")
    
    plt.plot(o_wmin, label = "oct white")
    plt.legend()
    plt.title('whitekrefs')
#%%    
    
    
    
    ref = RO.get_reflectance(QAQC=True)
    po = RO.process()
    po.update_root_dir('C:/Users/russj/Desktop/GUIs/Full_test')
    po.save_all()
    #%%
    import spectral as sp
    sp.imshow(RO.reflectance)
    sp.imshow(po.datasets['cropped'].data)
